...
func (tb *TransactionBuffer) clpaAllocation(AddShard *AccountLocation, ShardNum int) (map[int]float64, int) {
	CrossEdges := 0
...
func CLPA() {
	...
	parseFile("graph_clpa.txt", AddrShard)
	...
	fmt.Println(cost)
	fmt.Println("跨分片通信比例", float64(TotalCrossEdges)/float64(Params.ExtraDataSize))
