...
func (tb *TransactionBuffer) FG_LPA_Allocation(AddShard *AccountLocation, ShardNum int) (map[int]float64, int) {
	CrossEdges := 0
...
func FG_LPA() {
	...
	for _, transactionBuffer := range TransactionSets {
		ShardLoad, Crossedges := transactionBuffer.FG_LPA_Allocation(AddrShard, ShardNum)
